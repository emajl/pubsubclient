   ...
   if (!connected()) {
    ...
    Serial.println(result);
      ...
      if (result) {
         ...
         uint16_t len = 4;
         buffer[3] = 0;
         Serial.println("pubsub efter readPacket()");
         if (len == 4 && buffer[3] == 0) {
...
uint16_t PubSubClient::readPacket(uint8_t* lengthLength) {
   Serial.println("readPacket1");
   uint16_t len = 0;
   buffer[len++] = readByte();
   Serial.println("readpacket1.1");
   uint32_t multiplier = 1;
   uint16_t length = 0;
   uint8_t digit = 0;
   do {
      Serial.println("readPacket2");
      digit = readByte();
      buffer[len++] = digit;
   ...
   *lengthLength = len-1;
   Serial.println("readPacket3");
   for (uint16_t i = 0;i<length;i++)
   {
      if (len < MQTT_MAX_PACKET_SIZE) {
         Serial.println("readPacket4");
         buffer[len++] = readByte();
      } else {
         Serial.println("readPacket5");
         readByte();
         len = 0; // This will cause the packet to be ignored.
      }
   }
Serial.println("readPacket6");
   return len;
}
